...
public class ProcessSourceView {
    ...
    private static Set<String> mediators = new HashSet<>(Arrays.asList("log", "call", "enqueue", "send", "loopback",
            ...
            "builder", "propertyGroup", "jsontransform", "dataServiceCall", "NTLM"));
            ...
            if (tempTag.isStartTag()) { // 14
                ...
                if (prev != null && prev.getTagType() != 8) {
                    ...
                    if (!intermediary.contains(tempTag.getqName()) && ((tempTag.getTagType() == 3)
                            ...
                            || (artifacts.contains(tempTag.getqName()) && (currentMediator == null || (currentMediator != null &&  !currentMediator.getqName().equals("payloadFactory"))))  
                            || (currentMediator != null && currentMediator.getqName().equals("property") && intermediary.size() > 0))) {
